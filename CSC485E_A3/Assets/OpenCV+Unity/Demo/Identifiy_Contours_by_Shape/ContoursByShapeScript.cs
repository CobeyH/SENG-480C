namespace OpenCvSharp.Demo
{
	...
	public class ContoursByShapeScript : MonoBehaviour {
		...
		[SerializeField] private int circlePointThreshold = 15;
		public bool refresh;
		...
		private void DetectShape()
        {
			...
			foreach (Point[] contour in contours)
			{
				...
				else if (approx.Length >= circlePointThreshold)
				{
					shapeName = "Circle";
					color = new Scalar(0, 255, 255);
				}
				else
					Debug.Log($"Undefined shape with approx {approx.Length} points");
